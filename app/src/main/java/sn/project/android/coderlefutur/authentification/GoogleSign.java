...
import sn.project.android.coderlefutur.MenuApp;
...
public class GoogleSign extends AppCompatActivity {
    ...
    private static final String TAG = GoogleSign.class.getSimpleName();

    private static final int RC_SIGN_IN = 901;//google sign in request code
 ...
 /**           Personne personne = new Personne (account.getGivenName (),account.getFamilyName (),account.getEmail ());
            dataBaseManager.insertPersonne (personne);

            List<Personne> coursList = dataBaseManager.readPersonnes ();
            for(Personne m : coursList){
                Toast.makeText (this,m.getPrenom (),Toast.LENGTH_LONG).show ();
            }
  */


            /**
             * POur verifier si l'utlisateur s'est deja connecté
             */

            sharedPreference.saveBoolean (context,true);

            /**
             *Redirection au menu de l'application si la connexion reussie
             */
            ...
            sharedPreference.saveBoolean (context,false);
        ...
        mGoogleSignInClient.signOut()
                .addOnCompleteListener(this, new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        Toast.makeText(GoogleSign.this, "Google Sign Out done.", Toast.LENGTH_SHORT).show();
                        revokeAccess();
                    }
                });
    }
    ...
    private void revokeAccess() {
        mGoogleSignInClient.revokeAccess()
                .addOnCompleteListener(this, new OnCompleteListener<Void> () {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        Toast.makeText(GoogleSign.this, "Google access revoked.", Toast.LENGTH_SHORT).show();
                        //getProfileInformation(null);
                    }
                });
    }
